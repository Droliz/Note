...
`var`命令会发生“变量提升”现象，即变量可以在声明之前使用，值为`undefined`。这种现象多多少少是有些奇怪的，为了纠正这种现象，`let`命令改变了语法行为，它所声明的变量一定要在声明后使用
...
相比于 ES5 声明变量的两种方法（`var`、`function`），ES6 新添加了 `let` 、`const`、`import`、`class`，相较于 ES5，ES6 有 6 种声明变量的方法
...
只要等号两边的模式相匹配，那么就按照对应位置赋值

```javascript
// 表示从数组种提取值，按照对应位置为变量赋值
let [a, b, c] = [1, 2, 3];

// 相当于分别将 1 2 3 赋值给 a b c
...
事实上，只要某种数据结构具有 [Iterator 接口](#iterator接口) ，都可以采用数组形式的解构赋值
...
字符串添加了 [Iterator 接口](#iterator-接口)，所以字符串的遍历可以使用 `for...of` 的形式，这种形式可以识别大于`0xFFFF`的码点
...
ES6采用模板字符串，用反引号来标记，它可以当作普通字符串使用，也可以用来定义多行字符串，或者在字符串中嵌入变量，
...
模板字符串用`<% ... %>`放置 JavaScript 代码，使用`<%= ... %>`输出 JavaScript 表达式
...
`matchAll()`方法返回一个[正则表达式](#字符串的正则方法)在当前字符串的所有匹配
...
如果参数位置超出了字符串范围，`at()`返回`undefined`，该方法来自[数组](#数组拓展)添加的`at()`方法
...
装饰器（Decorator）是一种函数，写成 `@ + 函数名`。它可以放在类和类方法的定义前面
